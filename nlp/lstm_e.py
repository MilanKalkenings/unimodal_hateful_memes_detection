...
class LSTMEWrapper:
    """
    A wrapper for LSTMEClassifier, that enables the core functionalities of the network.
    """
    ...
    def evaluate_hyperparameters(self, folds, parameters, verbose=2):
        ...
        for fold_id in range(len(folds)):
            ...
            model = LSTMEClassifier(feats_per_time_step=feats_per_time_step,
    ...
    def predict(self, model, data, parameters, vocab):
        ...
        :param LSTMEClassifier model: a trained LSTMEClassifier
    ...
    def fit(self, train_data, best_parameters, verbose=2):
        """
        Trains an LSTMEClassifier on train_data using a set of parameters.
...
lstme_wrapper = LSTMEWrapper()
# print(lstme_wrapper.evaluate_hyperparameters(folds=train_folds, parameters=parameters))
train_data = train_folds[0]
for i in range(1, len(train_folds) - 1):
    pd.concat([train_data, train_folds[i]], axis=0)
fitted = lstme_wrapper.fit(train_data=train_data, best_parameters=parameters, verbose=1)
vocab = fitted["vocab"]
best_rnne_clf = fitted["model"]
print("PERFORMANCE ON TEST:")
lstme_wrapper.predict(model=best_rnne_clf, data=test_fold, parameters=parameters, vocab=vocab)
